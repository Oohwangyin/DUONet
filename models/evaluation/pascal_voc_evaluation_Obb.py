...
class PascalVOCDetectionEvaluator(DatasetEvaluator):
    ...
    def evaluate(self):
        ...
        results_file = os.path.join(self.output_dir, "eval_results.txt") 
        
        # 获取当前迭代次数(从输出目录名称解析)
        import re
        curr_iter = None
        model_file = [f for f in os.listdir(self.output_dir) if f.startswith("model_") and f.endswith(".pth")]
        if model_file:
            match = re.search(r"model_(\d+)", model_file[0])
            if match:
                curr_iter = int(match.group(1))
        
        # 准备要写入的内容
        content = f"\nIteration {curr_iter}:\n" if curr_iter is not None else "\nEvaluation:\n"
        content += table # table 是之前代码中的表格内容
...
def parse_rec(filename, known_classes):
    ...
    #filename = "/home/quchenyu/DUONet/make/15+5/Annotations"  DOSR test set path
    ...
    for obj in tree.findall("object"):
        ...
        #obj_struct["bbox"] = [
            ...
            ## angle
            #180 - math.degrees(float(bbox.find("angle").text))
        #]
        #objects.append(obj_struct)
